...
#endif
bool EvaluateQuery::exit_ = false;
...
size_t EvaluateQuery::QuickSIStyleForRL(const Graph *data_graph, const Graph *query_graph, ui **candidates, ui *candidates_count, ui *order, ui * pivot,
                            size_t output_limit_num, ui * add_pool_depth, size_t & search_node, size_t & intersection_time) {
    size_t embedding_cnt = 0;
    int cur_depth = 0;
    int query_num = query_graph->getVerticesCount();
    int max_depth = 2 * query_num;
    VertexID start_vertex = order[0];

    // make bn base on new matching order list
    // Generate the bn.
    ui **bn;
    ui *bn_count;
    ...
    std::vector<bool> visited_query_vertices(query_num, false);
    visited_query_vertices[start_vertex] = true;
    for (ui i = 1; i < max_depth; ++i) {
        VertexID cur_vertex = order[i];
        if(visited_query_vertices[cur_vertex])
            continue;

        ui nbrs_cnt;
        const ui *nbrs = query_graph->getVertexNeighbors(cur_vertex, nbrs_cnt);
        for (ui j = 0; j < nbrs_cnt; ++j) {
            VertexID nbr = nbrs[j];

            if (visited_query_vertices[nbr] && nbr != pivot[i]) {
                bn[i][bn_count[i]++] = nbr;
            }
        }
    ...
    // control setting for rl
    std::vector<bool> add_pool_operation(query_num, false);
    std::vector<bool> expansion_operation(query_num, false);
    int depth = -1;
    //exeute operation
    OP_execute(data_graph, query_graph, candidates, candidates_count, order, output_limit_num, depth, add_pool_operation, expansion_operation, bn, bn_count,
                idx, idx_count, embedding, valid_candidate, visited_vertices, search_node, intersection_time, max_depth, embedding_cnt, pivot, add_pool_depth);
...
void EvaluateQuery::OP_execute(const Graph *data_graph, const Graph *query_graph, ui **candidates, ui *candidates_count, ui *order,
                            size_t output_limit_num, int depth, std::vector<bool> & add_pool_operation, std::vector<bool> & expand_operation, 
                            ui ** & bn, ui * & bn_count, ui * & idx, ui *& idx_count, ui * & embedding, VertexID ** & valid_candidate, bool * & visited_vertices, size_t & search_node, size_t & intersection_time,
                            const ui max_depth, size_t & embedding_cnt, ui * pivot, ui * add_pool_depth){
    if(exit_ ||  embedding_cnt >= output_limit_num)
        return;
    depth += 1;
    ui query_vertex = order[depth];
    if(add_pool_operation[query_vertex])
    {
        Execute_expansion(data_graph, query_graph, candidates, candidates_count, order, output_limit_num, depth, add_pool_operation, expand_operation, bn, bn_count,
                            idx, idx_count, embedding, valid_candidate, visited_vertices, search_node, intersection_time, max_depth, embedding_cnt, pivot, add_pool_depth);
    }
    else
    {
        Execute_add_pool(data_graph, query_graph, candidates, candidates_count, order, output_limit_num, depth, add_pool_operation, expand_operation, bn, bn_count,
                            idx, idx_count, embedding, valid_candidate, visited_vertices, search_node, intersection_time, max_depth, embedding_cnt, pivot, add_pool_depth);
    }
    depth -= 1;
    if(embedding_cnt > output_limit_num)
...
void EvaluateQuery::Execute_add_pool(const Graph *data_graph, const Graph *query_graph, ui **candidates, ui *candidates_count, ui *order,
                            size_t output_limit_num, int depth, std::vector<bool> & add_pool_operation, std::vector<bool> & expand_operation, 
                            ui ** & bn, ui * & bn_count, ui * & idx, ui *& idx_count, ui * & embedding, VertexID ** & valid_candidate, bool * & visited_vertices, size_t & search_node, size_t & intersection_time,
                            const ui max_depth, size_t & embedding_cnt, ui * pivot, ui * add_pool_depth){
    if(exit_ ||  embedding_cnt >= output_limit_num)
        return;
    ui current_vertex = order[depth];
#if defined(RL_DEBUG) || defined(Subgraph_DEBUG)
    std::cout << std::endl << "Depth: " << depth << std::endl;
    std::cout << "Add pool: " << current_vertex << std::endl;
#endif
    if(depth == 0)
    {
        idx[depth] = 0;
        idx_count[depth] = candidates_count[current_vertex];
        std::copy(candidates[current_vertex], candidates[current_vertex] + candidates_count[current_vertex],
                valid_candidate[depth]);       
    }
    else
    {
        idx[depth] = 0;
        intersection_time += bn_count[depth] + 1;
        generateValidCandidates(query_graph, data_graph, (uint)depth, embedding, idx_count, valid_candidate,
                                                visited_vertices, bn, bn_count, order, pivot);
    }
#if defined(RL_DEBUG) || defined(Subgraph_DEBUG)
    std::cout << "create candidate: " << idx_count[depth] << std::endl;
#endif
    add_pool_operation[current_vertex] = true;
#if defined(FIRST_MAPPING)
    if(idx_count[depth] == 1)
    {
        embedding[current_vertex] = valid_candidate[depth][idx[depth]];
        idx[depth]++;
        visited_vertices[embedding[current_vertex]] = true;
        expand_operation[current_vertex] = true;
        OP_execute(data_graph, query_graph, candidates, candidates_count, order, output_limit_num, depth, add_pool_operation, expand_operation, bn, bn_count,
                idx, idx_count, embedding, valid_candidate, visited_vertices, search_node, intersection_time, max_depth, embedding_cnt, pivot, add_pool_depth);
        idx[depth]--;
        visited_vertices[embedding[current_vertex]] = false;     
        expand_operation[current_vertex] = false;   
    }
    else
    {
#endif
    ...
    if(embedding_cnt > output_limit_num)
        return;
    add_pool_operation[current_vertex] = false;
}

void EvaluateQuery::Execute_expansion(const Graph *data_graph, const Graph *query_graph, ui **candidates, ui *candidates_count, ui *order,
                            size_t output_limit_num, int depth, std::vector<bool> & add_pool_operation, std::vector<bool> & expand_operation, 
                            ui ** & bn, ui * & bn_count, ui * & idx, ui *& idx_count, ui * & embedding, VertexID ** & valid_candidate, bool * & visited_vertices, size_t & search_node, size_t & intersection_time,
                            const ui max_depth, size_t & embedding_cnt, ui * pivot, ui * add_pool_depth){
    ui current_vertex = order[depth];
    ui candidate_depth = add_pool_depth[current_vertex];
#ifdef FIRST_MAPPING
    bool all_ready_expand = true;
    for(const auto & item :expand_operation)
    {
        if(!item)
            all_ready_expand = item;
    }
    if(all_ready_expand)
    {
        embedding_cnt++;
    ...
    if(expand_operation[current_vertex])
    {
        OP_execute(data_graph, query_graph, candidates, candidates_count, order, output_limit_num, depth, add_pool_operation, expand_operation, bn, bn_count,
            idx, idx_count, embedding, valid_candidate, visited_vertices, search_node, intersection_time, max_depth, embedding_cnt, pivot, add_pool_depth); 
        return;
...
#if defined(RL_DEBUG) || defined(Subgraph_DEBUG)
    std::cout << std::endl << "Depth: " << depth << std::endl;
    std::cout << "Expansion: " << current_vertex << std::endl;
    std::cout << "Candidate make depth: " << candidate_depth << std::endl;
    std::cout << "candidate: ";
    for(uint idx = 0; idx < idx_count[candidate_depth]; idx++)
    {
        std::cout << valid_candidate[candidate_depth][idx] << " ";
    }
    std::cout << std::endl;
    std::cout << "mapping get: ";
    for(uint idx = 0; idx < max_depth / 2; idx++)
    {
    ...
    while(idx[candidate_depth] < idx_count[candidate_depth])
    {
...
#if defined(Subgraph_DEBUG)
        std::cout << "expansion: " << current_vertex << std::endl;
        std::cout << "v: " << v << std::endl;

        std::cout << "mapping before: ";
        for(uint idx = 0; idx < max_depth / 2; idx++)
        {
            std::cout << embedding[idx] << " ";
        }
        ...
        visited_vertices[v] = false;
#if defined(Subgraph_DEBUG)
        embedding[current_vertex] = 0;
#endif
    }
    expand_operation[current_vertex] = false;
    idx[candidate_depth] = 0;
}
