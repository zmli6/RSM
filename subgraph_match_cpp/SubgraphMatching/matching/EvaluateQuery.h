...
public:
    ...
    static size_t
    QuickSIStyleForRL(const Graph *data_graph, const Graph *query_graph, ui **candidates, ui *candidates_count, ui *order, ui * pivot,
                            size_t output_limit_num, ui * add_pool_depth, size_t & search_node, size_t & intersection_time);

    static void OP_execute(const Graph *data_graph, const Graph *query_graph, ui **candidates, ui *candidates_count, ui *order,
    ...
    static void Execute_add_pool(const Graph *data_graph, const Graph *query_graph, ui **candidates, ui *candidates_count, ui *order,
                            size_t output_limit_num, int depth, std::vector<bool> & add_pool_operation, std::vector<bool> & expand_operation, 
                            ui ** & bn, ui * & bn_count, ui * & idx, ui *& idx_count, ui * & embedding, VertexID ** & valid_candidate, bool * & visited_vertices, size_t & search_node, size_t & intersection_time,
                            const ui max_depth, size_t & embedding_cnt, ui * pivot, ui * add_pool_depth);

    static void Execute_expansion(const Graph *data_graph, const Graph *query_graph, ui **candidates, ui *candidates_count, ui *order,
                            size_t output_limit_num, int depth, std::vector<bool> & add_pool_operation, std::vector<bool> & expand_operation, 
                            ui ** & bn, ui * & bn_count, ui * & idx, ui *& idx_count, ui * & embedding, VertexID ** & valid_candidate, bool * & visited_vertices, size_t & search_node, size_t & intersection_time,
                            const ui max_depth, size_t & embedding_cnt, ui * pivot, ui * add_pool_depth);
    static size_t
